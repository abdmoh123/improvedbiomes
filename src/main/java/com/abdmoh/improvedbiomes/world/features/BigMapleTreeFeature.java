...
public class BigMapleTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
    private static final BlockState LOG = ModBlocks.MAPLE_LOG.getDefaultState();
    private static final BlockState LEAF = ModBlocks.MAPLE_LEAVES.getDefaultState();

    public BigMapleTreeFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> configFactoryIn, boolean doBlockNotifyIn) {
        super(configFactoryIn, doBlockNotifyIn);
        setSapling(ModBlocks.MAPLE_SAPLING);
    }
    ...
    private void makeFoliage(IWorldGenerationReader worldIn, int p_208525_2_, BlockPos pos, List<BigMapleTreeFeature.FoliageCoordinates> p_208525_4_, MutableBoundingBox p_208525_5_, Set<BlockPos> changedBlocks) {
        for(BigMapleTreeFeature.FoliageCoordinates bigtreefeature$foliagecoordinates : p_208525_4_) {
    ...
    private void makeBranches(Set<BlockPos> p_208524_1_, IWorldGenerationReader p_208524_2_, int p_208524_3_, BlockPos p_208524_4_, List<BigMapleTreeFeature.FoliageCoordinates> p_208524_5_, MutableBoundingBox p_208524_6_) {
        for(BigMapleTreeFeature.FoliageCoordinates bigtreefeature$foliagecoordinates : p_208524_5_) {
    ...
    public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldIn, Random rand, BlockPos position, MutableBoundingBox boundsIn) {
        ...
        if (i == -1) {
            ...
            int i1 = i - 5;
            List<BigMapleTreeFeature.FoliageCoordinates> list = Lists.newArrayList();
            list.add(new BigMapleTreeFeature.FoliageCoordinates(position.up(i1), l));
                    ...
                    for(int j1 = 0; j1 < k; ++j1) {
                        ...
                        if (this.makeLimb(changedBlocks, worldIn, blockpos, blockpos1, false, boundsIn) == -1) {
                            ...
                            if (this.makeLimb(changedBlocks, worldIn, blockpos2, blockpos, false, boundsIn) == -1) {
                                list.add(new BigMapleTreeFeature.FoliageCoordinates(blockpos, blockpos2.getY()));
                            }
                        }
                    }
                }
